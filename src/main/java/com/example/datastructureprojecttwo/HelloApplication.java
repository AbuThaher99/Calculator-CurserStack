...
public class HelloApplication extends Application {
	...
	public static Double evaluatePostfixExpression(String postfixExpr) {
		...
		for (int i = 0; i < items.length; i++) {
			try {
				stack.push(Double.valueOf(items[i]));
	...
	public static String postfix(String x) {
		...
		for (int i = 0; i < y.length; i++) {
			if (y[i].equals("(")) {
				...
				a.push(y[i]);
			} else if (y[i].contains("sin") || y[i].contains("cos") || y[i].contains("log") || y[i].contains("tan")
					|| y[i].contains("ln") || y[i].contains("e")) {
				a.push(y[i]);
			} else {
				result += " " + y[i];
			}
			...
			but35.setOnAction(e -> {
				try {
					if (checkParameters(Tx.getText()) == true) {
						...
						if (String.valueOf(evaluatePostfixExpression(Tx1.getText())).equals("Infinity")) {
							Tx2.setText("");
							Tx1.setText("");
							
							Alert alert = new Alert(Alert.AlertType.NONE, "Can Not Divid by Zero"+"\n"+"InValid Equation", ButtonType.OK);
							if (alert.showAndWait().orElse(ButtonType.NO) == ButtonType.YES) {}
						}

					} else {
						Alert alert = new Alert(Alert.AlertType.NONE, "Missing parentheses", ButtonType.OK);
						if (alert.showAndWait().orElse(ButtonType.NO) == ButtonType.YES) {}
					...
					Tx2.setText("");
					Tx1.setText("");
					Alert alert = new Alert(Alert.AlertType.NONE, "InValid Equation", ButtonType.OK);
					if (alert.showAndWait().orElse(ButtonType.NO) == ButtonType.YES) {}
				}
